...
class _BudgetPageState extends State<BudgetPage> {
  ...
  Future<void> _loadBudgetData() async {
    ...
    final List<String> cats =
        prefs.getStringList('custom_categories') ?? defaultCats;
  ...
  String _format(double amount) => amount
      .toStringAsFixed(0)
      .replaceAllMapped(
      ...
      body: BlocBuilder<ExpenseBloc, ExpenseState>(
        builder: (context, state) {
          ...
          if (state is ExpenseLoaded) {
            ...
            final monthlyExpenses = state.expenses
                .where(
            ...
            final double totalPercent = totalMonthlyBudget > 0
                ? (totalSpent / totalMonthlyBudget).clamp(0.0, 1.0)
                : 0.0;
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Expanded(
                child: Text(
                  l10n.spentOfBudget((percent * 100).toInt().toString()),
                  maxLines: 1,
  ...
  Widget _buildCategoryBudgetCard(
    ...
    final double percent = (spent / displayLimit).clamp(0.0, 1.0);
    final bool isOverBudget = spent > limit && limit > 0;

    Color barColor;
    final titleLower = title.toLowerCase();
    if (titleLower == 'oziq-ovqat' ||
        titleLower == 'food' ||
        titleLower == 'продукты' ||
        titleLower == l10n.food.toLowerCase()) {
      barColor = const Color(0xFF0D9488);
    } else if (titleLower == 'transport' ||
        titleLower == 'транспорт' ||
        titleLower == l10n.transport.toLowerCase()) {
      barColor = Colors.blue;
    } else if (titleLower == 'xaridlar' ||
        titleLower == 'shopping' ||
        titleLower == 'покупки' ||
        titleLower == l10n.shopping.toLowerCase()) {
      barColor = Colors.purple;
    } else if (titleLower == 'to\'lovlar' ||
        titleLower == 'tolovlar' ||
        titleLower == 'payments' ||
        titleLower == 'платежи' ||
        titleLower == l10n.payments.toLowerCase()) {
      barColor = Colors.orange;
    } else if (titleLower == 'salomatlik' ||
        titleLower == 'health' ||
        titleLower == 'здоровье' ||
        titleLower == l10n.health.toLowerCase()) {
    ...
    return GestureDetector(
      onTap: () => _showEditDialog(
        l10n.categoryBudgetAllocated(localizedTitle),
        limit,
        (val) => _saveCategoryLimit(title, val),
        l10n,
      ),
      child: Container(
            ...
            Row(
              children: [
                ...
                FittedBox(
                  fit: BoxFit.scaleDown,
                  child: Text(
                    "${_format(spent)} / ${_format(limit)}",
                    style: TextStyle(
                      color: isOverBudget ? Colors.red : const Color(0xFF64748B),
                      fontSize: 13,
